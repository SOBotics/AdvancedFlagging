...
(() => {
  ...
  function getExistingElements() {
    if (!isQuestionPage && !isNatoPage && !isFlagsPage && !isSearch)
      return;
    let elements;
    if (isNatoPage) {
  ...
  function getPostType(element) {
    return element.classList.contains("question") || element.querySelector('.question-hyperlink, .s-link:not([href^="/collectives"])') ? "Question" : "Answer";
  }
  ...
  function setupDefaults() {
    ...
    cachedFlagTypes.forEach((cachedFlag) => {
      if (cachedFlag.id !== 3 && cachedFlag.id !== 5)
        return;
  ...
  function getOptionsRow({ element, postId }) {
    ...
    return config.map(([text, cacheKey]) => {
  ...
  async function runOnNewTask(xhr) {
    ...
    if (xhr.status !== 200 || !regex.test(xhr.responseURL) || !document.querySelector("#answer"))
      return;
  ...
  function setupReview() {
    ...
    if (!watchReview || !isLqpReviewPage)
      return;
...
#advanced-flagging-configuration-modal > .s-modal--dialog {
  ...
  function setFlagWatch({ postId, flagged }, reporters) {
    ...
    addXHRListener((xhr) => {
      ...
      if (!watchFlags || autoFlagging || status !== 200 || !regex.test(responseURL))
        return;
  ...
  function setupPostPage() {
    ...
    if (linkDisabled || isLqpReviewPage)
      return;
    const page = getPage();
